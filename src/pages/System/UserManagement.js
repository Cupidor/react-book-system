...
class Index extends PureComponent {
  ...
  getAllUsers = async () => {
    ...
    if (res.code === '0000') {
      ...
      for (let item of res.result) {
        ...
        obj.is_manager = item.is_manager;
  ...
  createUser = () => {
    this.setState(
      ...
      () => {
        this.setIsUserModalVisible(true);
      },
    );
  };
  ...
  render() {
    const { groups, isNewVisible, type, users, isNewUserVisible, userType } = this.state;
    const columns = [
      {
        title: '创建时间',
        ...
        <div className={global.MyContent}>
          <div className={global.MyHeader}>
            <div className={global.MyTitle}>
              <Text style={{ fontSize: 16 }}>用户管理</Text>
            </div>
            <Space>
